...
- `summary`：**单行**中文，一句话说清画面里看得见的东西（主体、地点、动作、画面类型），不要写你的解读。GetCut 会直接拿它当素材说明用，写得准才配得准。上限 240 字。
- `tags`：2 到 5 个中文短标签，每个不超过 32 字，最多 8 个。
- `source_url`：素材所在那一页的地址（不是网站首页），方便回溯来源，能填就填。
- `script_anchor`：**从口播文案里原样摘一小句**，表示这个素材适合配在这句话附近，可以留空。注意这只是建议：最终分几个镜头、素材落在第几镜由 GetCut 决定，不要试图在这里排完整的分镜表。上限 240 字。
...
- 已经按第三步裁好的视频**不要**写 `clip_start_seconds` / `clip_end_seconds`。只有交的是一段较长的原片、希望 GetCut 从中取某一段时才写。
- 交长原片却写了 `summary` 和 `tags`、又没写选段时间，GetCut 会认为你已经挑好了镜头，直接从第 0 秒开始取，大概率取到片头。
- `manifest.json` 里声明的文件必须真实存在。反过来，磁盘上多出来的文件不写进 manifest 也能用，GetCut 会自动补描述。
