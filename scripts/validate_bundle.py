...
校验规则与 GetCut 服务端的严格协议对齐：报错的项服务端一定会拒收，告警的项服务端会
...
# 常见的误写：这些都是用户在 GetCut 里渲染前才决定的参数，属于 render.json 而不是内容协议。
...
def validate_content(content: dict[str, Any], report: Report) -> None:
    ...
    for key in sorted(set(content) - CONTENT_KEYS):
        if key in ("video_format", "aspect_ratio"):
            report.error(
                f"content.json 不要写 {key!r}，成片形态由用户在 GetCut 渲染界面上选。"
    ...
    if not isinstance(title, str) or not title.strip():
        report.error("title 不能为空，用户在 GetCut 里靠它认出这个素材包")
...
def _check_entry_text(entry: dict[str, Any], label: str, report: Report) -> None:
    ...
    if not isinstance(summary, str) or not summary.strip():
        report.warn(f"{label} 没写 summary，GetCut 会用视觉模型自己给它补描述")
    else:
        if "\n" in summary:
...
def check_foreign_declarations(bundle_dir: Path, report: Report) -> None:
    ...
    if (bundle_dir / "render.json").is_file():
        report.warn(
            ...
            "由用户在 GetCut 里渲染前自己选，不该由你产出，别打进 zip"
        )
